...
function countVowels(str) {
  return (str.toLocaleLowerCase().match(/[aeiouy]/g) || []).length;
}
...
function isPalindrome(str) {
  let result = false;
  const newStr = str.toLowerCase().replaceAll(/[?.,!]/g, '');
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < newStr.length; i++) {
    result = newStr[i] === newStr[newStr.length - 1 - i];
  }
  return result;
}
...
function invertCase(str) {
  ...
  const strArr = str.split('');
  strArr.forEach((el) => {
    result += el.toUpperCase() === el ? el.toLowerCase() : el.toUpperCase();
  });
  return result;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (index > -1) {
      const res =
        alf[
          index + 13 <= alf.length - 1
            ? index + 13
            : Math.abs(alf.length - index - 13)
        ];
      if (str[i].toUpperCase() === str[i]) result += res.toUpperCase();
      else result += res;
    } else result += str[i];
  }
  return result;
